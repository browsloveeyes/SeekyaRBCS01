...
namespace Seekya
{
    class DbOperate
    {
        ...
        //更改记录
        public bool ModifyRecord(string originDate, string originTime, string hospitalName, string roomName, string instrumentModel, string name, string sex, string age, string number, string CO, string CO2, string RBC, string rbConcentration, string sendDoctor, string reviewDoctor, string checkDoctor, string firstCheck, string typeName,string time, string date, string remark1, string remark2)
        {
            DeleteRecord(originDate, originTime);//删除原始的记录
            OleDbConnection aConnection = new OleDbConnection("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + System.AppDomain.CurrentDomain.BaseDirectory + "Data\\checkDb.mdb");
            string strSql = "Insert into " + originDate + " (医院名称,科室名称,仪器型号,姓名,性别,年龄,住院号,CO,CO2,红细胞寿命,血红蛋白浓度,送检医生,复核医生,报告医生,初步诊断,样本类型,时间,日期,备注1,备注2) values ('" + hospitalName + "','" + roomName + "','" + instrumentModel + "','" + name + "','" + sex + "','" + age + "','" + number + "','" + CO + "','" + CO2 + "','" + RBC + "','" + rbConcentration + "','" + sendDoctor + "','" + reviewDoctor + "','" + checkDoctor + "','" + firstCheck + "','" +typeName+ "','"+ time + "','" + date + "','" + remark1 + "','" + remark2 + "')";
